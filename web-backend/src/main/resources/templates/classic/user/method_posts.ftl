                        ...
                        <#list results.content as row>
                            <li class="list-group-item" el="loop-${row.id}">
                                ...
                                <div class="pull-right hidden-xs">
                                    <#if owner>
                                        <a class="act_edit" href="javascript:void(0);" data-evt="edit" data-id="${row.articleBlogId}" data-toggle="tooltip" title="编辑文章">
                                            <i class="icon icon-note"></i>
                                        </a>
                                        <a class="act_delete" href="javascript:void(0);" data-evt="trash" data-id="${row.articleBlogId}" data-toggle="tooltip" title="删除文章">
...
$(function() {
	...
	$('a[data-evt=trash]').click(function () {
		...
		layer.confirm('确定删除此项吗?', {
			...
			jQuery.getJSON('${base}/post/delete/' + articleBlogId, function (ret) {
	...
	$('a[data-evt=edit]').click(function () {
		var articleBlogId = $(this).attr('data-id');
		window.location.href='${base}/post/editing?articleBlogId=' + articleBlogId;
	});
})
</script>
</@layout>