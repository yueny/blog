...
@EnableAsync
@Slf4j
public class UploadXmlAbilityService implements IUploadXmlAbilityService {
	/**
	 * 先从配置中心获取。取不到使用默认值
	 *
	 * @return
	...
	private UploadConfigModelData uploadConfig(String xmlSettingPaths) {
		...
		try {
			reader = new InputStreamReader(UploadXmlAbilityService.class.getResourceAsStream(xmlSettingPaths), "UTF-8");
